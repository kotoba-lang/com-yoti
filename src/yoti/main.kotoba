(ns yoti.main
  "Kotodama WASM entrypoint for the Yoti clean-room actor (L4) — Clojure port.
  ...
  facts (`yoti.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"yoti.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "yoti-compat" :tier tier :entities entities} 200])
